...
def main():
    ...
    def drawGameStatusInfo():
        ...
        window.blit(playerImage, (
            healthBar.midright[0] - playerImage.get_width() // 2, healthBar.centery - playerImage.get_height() // 2))
        healthText = font.render(str(round(player.health / 5, 1)) + "%", 100, white)
        window.blit(healthText,
                    (damageBar.centerx - healthText.get_width() // 2, damageBar.centery - healthText.get_height() // 2))
